...
librarianController.bookIssued = async (req, res) => {
  try {
      ...
      res.status(200).json({message:"Requested books fetched successfully",requests});
    } catch (err) {
...
librarianController.approveRequest = async (req, res) => {
  ...
  try {
    // Step 1: Find the borrow request
    ...
    // Step 2: Find the associated book
    ...
    // Step 5: Update the borrow request
...
librarianController.approveReturnRequest = async (req, res) => {
  ...
  try {
    ...
    // Update book availableCopies only if it won't exceed totalCopies
    ...
    res.status(200).json({ message: "Book return approved and updated successfully" });
  } catch (error) {
